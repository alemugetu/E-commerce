...
const inputCls =
  'w-full rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-950/80 px-3 py-2 text-sm text-slate-900 dark:text-slate-100 placeholder:text-slate-400 dark:placeholder:text-slate-600 outline-none focus:border-emerald-500 dark:focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500/40 transition';
const labelCls =
  'mb-1 block text-[10px] font-bold uppercase tracking-[0.2em] text-slate-600 dark:text-slate-400';
...
const SellerProducts = () => {
  ...
  const columns = [
    {
      ...
      render: (val, row) => (
        <div>
          <p className="font-semibold text-slate-900 dark:text-slate-100 truncate max-w-[160px]">{val}</p>
          <p className="text-[11px] text-slate-600 dark:text-slate-500 truncate max-w-[160px]">{row.brand}</p>
        </div>
      ),
    },
    {
      ...
      render: (val, row) => (
        <div>
          <p className="text-slate-900 dark:text-slate-200">{Number(val).toLocaleString()} ETB</p>
          {row.discount_price && (
        ...
        <div className="flex justify-end gap-2">
          ...
          <button
            ...
            className={`rounded-lg border px-2.5 py-1.5 text-xs font-semibold transition ${
              row.is_active
                ? 'border-rose-500/20 dark:border-rose-500/20 bg-rose-500/10 dark:bg-rose-500/10 text-rose-700 dark:text-rose-400 hover:bg-rose-500/20 dark:hover:bg-rose-500/20'
                : 'border-emerald-500/20 dark:border-emerald-500/20 bg-emerald-500/10 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 hover:bg-emerald-500/20 dark:hover:bg-emerald-500/20'
            }`}
          >
  ...
  return (
    <div className="space-y-6">
      ...
      {/* ── Header ── */}
      <div>
        <p className="text-xs font-bold uppercase tracking-[0.3em] text-emerald-600 dark:text-emerald-400">Catalog</p>
        <h1 className="mt-1 text-2xl font-black text-slate-900 dark:text-slate-100">Products Management</h1>
        ...
        <section className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-slate-100 dark:bg-slate-900/70 p-5 sm:p-6 h-fit">
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              {editingId && (
                <p className="text-xs text-emerald-600 dark:text-emerald-400 mt-0.5">ID #{editingId}</p>
              )}
            </div>
            {editingId && (
              <button
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              ...
              <div>
                <label className={labelCls}>
                  Slug <span className="normal-case font-normal text-slate-600 dark:text-slate-500">(auto if blank)</span>
                </label>
            ...
            <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
              <label className="flex items-center gap-2 text-sm text-slate-700 dark:text-slate-300 cursor-pointer">
                <input type="checkbox" name="is_available" checked={formData.is_available}
                  onChange={handleChange}
                  className="h-4 w-4 rounded border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-950 accent-emerald-500" />
                Available for sale
              </label>
              <div className="flex-1 sm:max-w-xs">
                ...
                <input
                  ...
                  className="block w-full text-xs text-slate-600 dark:text-slate-400 file:mr-3 file:rounded-full file:border-0 file:bg-emerald-600/20 file:px-3 file:py-1.5 file:text-xs file:font-semibold file:text-emerald-700 dark:file:text-emerald-300 hover:file:bg-emerald-600/30 cursor-pointer"
                />
              </div>
            </div>
        ...
        <section className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-slate-100 dark:bg-slate-900/70 p-5 sm:p-6">
          <div className="mb-4 flex items-center justify-between gap-2">
            <div>
              ...
              <p className="text-xs text-slate-600 dark:text-slate-400 mt-0.5">All products in the catalog.</p>
            </div>
